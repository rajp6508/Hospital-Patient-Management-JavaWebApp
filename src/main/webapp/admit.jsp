...
<head>
    <meta charset="UTF-8">
    <title>Patient Admission - HPMS</title>
    ...
    <style>
        ...
        textarea { resize:vertical; height:80px; padding-left:12px !important; }
        ...
        .radio-group input[type="radio"] { margin-left:5px; margin-right:3px; transform:scale(1.1); }
...
<div class="container">
    <div class="register-title">
        ...
        <h2>Patient Admission Portal</h2>
    </div>

    <div class="welcome-text">
        <p>Admit patients efficiently with integrated digital records. Select existing or new patient to proceed.</p>
        <p><strong>Note:</strong> Admissions link to medical history for seamless care.</p>
    </div>
    ...
    <% } else { %>
        <form action="<%=request.getContextPath()%>/admit" method="post" id="admitForm" onsubmit="return validateForm()">

            <!-- Patient Type -->
            <div class="input-group radio-group">
                <label><i class="fas fa-user-check"></i> Patient Type:</label>
                <label><input type="radio" name="patientType" value="existing" checked onclick="togglePatientFields()"> Existing</label>
                <label><input type="radio" name="patientType" value="new" onclick="togglePatientFields()"> New</label>
            </div>

            <!-- Existing Patient -->
            <div id="existingFields" class="input-group">
                <label for="patientID">Patient ID *:</label>
                <i class="fas fa-id-card"></i>
                <input type="number" name="patientID" id="patientID" min="1" placeholder="e.g., 1">
            </div>

            <!-- New Patient -->
            <div id="newPatientFields" class="new-patient-fields" style="display:none;">
                <div class="input-group">
                    <label for="newName">Full Name *:</label>
                    <i class="fas fa-user"></i>
                    <input type="text" name="newName" id="newName" placeholder="John Doe">
                </div>
                <div class="input-group">
                    <label for="newAge">Age *:</label>
                    <i class="fas fa-calendar-alt"></i>
                    <input type="number" name="newAge" id="newAge" min="1" max="120" placeholder="30">
                </div>
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <input type="tel" name="newPhone" id="newPhone" placeholder="+1-123-456-7890">
                </div>
                <div class="input-group">
                    <label for="newEmail">Email:</label>
                    <i class="fas fa-envelope"></i>
                    <input type="email" name="newEmail" id="newEmail" placeholder="john@example.com">
                </div>
                <div class="input-group">
                    <label for="newAddress">Address:</label>
                    <i class="fas fa-map-marker-alt"></i>
                    <input type="text" name="newAddress" id="newAddress" placeholder="123 Main St">
                </div>
                <div class="input-group textarea-group">
                    <label for="newMedicalHistory"><i class="fas fa-notes-medical"></i> Medical History:</label>
                    <textarea name="newMedicalHistory" id="newMedicalHistory" placeholder="Past illnesses, allergies, medications, etc."></textarea>
                </div>
            </div>

            <!-- Admission Details -->
            <div class="input-group textarea-group">
                <label for="reason"><i class="fas fa-exclamation-triangle"></i> Admission Reason *:</label>
                <textarea name="reason" id="reason" required placeholder="Reason for admission..."></textarea>
            </div>
            <div class="input-group">
                <label for="ward"><i class="fas fa-hospital"></i> Ward *:</label>
            ...
            <div class="input-group">
                ...
                <select name="assignedDoctor" id="assignedDoctor" required>
                    ...
                    <%
                        ...
                        try (Connection conn = DriverManager.getConnection(jdbcURL, dbUser, dbPass);
                             ...
                             ResultSet rs = stmt.executeQuery("SELECT UserID, FullName FROM Users WHERE Role='Doctor' ORDER BY FullName")) {
                            while(rs.next()){
                                int doctorId = rs.getInt("UserID");
                                String doctorName = rs.getString("FullName");
                                out.println("<option value='"+doctorId+"'>"+doctorName+"</option>");
                            }
                        } catch(SQLException e) { out.println("<option>Error loading doctors</option>"); e.printStackTrace(); }
                    %>
                </select>
            </div>
            <div class="input-group">
                <label for="bedNumber"><i class="fas fa-bed"></i> Bed Number:</label>
                <input type="text" name="bedNumber" id="bedNumber" placeholder="e.g., Bed 101">
            </div>
            <div class="input-group">
                <label for="admissionDate"><i class="fas fa-calendar"></i> Admission Date:</label>
                <input type="date" name="admissionDate" id="admissionDate" value="<%=java.time.LocalDate.now()%>">
            </div>
...
function togglePatientFields(){
    ...
    document.getElementById('newPatientFields').style.display = (type==='new')?'block':'none';
}
...
function validateForm(){
    ...
    if(type==='existing'){
        ...
        if(!pid || pid<=0){ alert('Enter valid Patient ID'); return false; }
    } else {
        const name=document.getElementById('newName').value.trim();
        const age=document.getElementById('newAge').value;
        const phone=document.getElementById('newPhone').value.trim();
        if(!name||!age||!phone){ alert('Fill all required fields for new patient'); return false; }
    }
    const reason=document.getElementById('reason').value.trim();
    const ward=document.getElementById('ward').value;
    const doctor=document.getElementById('assignedDoctor').value;
    if(!reason||!ward||!doctor){ alert('Fill all admission fields'); return false; }
    return true;
}
</script>
